...
function switchView(viewName) {
    ...
    if (document.startViewTransition) {
        ...
        // Focus the page title heading programmatically for accessibility
        ...
        // Fallback accessibility focus shift
        const pageTitle = document.getElementById('page-title');
        if (pageTitle) {
            pageTitle.setAttribute('tabindex', '-1');
            pageTitle.focus({ preventScroll: true });
...
function renderIssueCards(review) {
    ...
    if (activeIssueFilter === 'positive') {
        ...
        positives.forEach(msg => {
            ...
            card.className = 'issue-card severity-positive';
            card.innerHTML = `
...
function selectIssue(issue) {
    ...
    document.querySelectorAll('.code-line-wrapper').forEach(l => {
        l.classList.remove('highlighted-concern', 'concern-critical', 'concern-warning', 'concern-suggestion');
    });
    ...
    if (issue.line) {
        ...
        if (lineWrapper) {
            lineWrapper.classList.add('highlighted-concern', `concern-${issue.severity}`);
...
function renderKanbanBoard() {
    ...
    Object.keys(containers).forEach(k => {
        ...
        document.getElementById(`count-col-${k.replace('_', '-')}`).innerText = '0';
    });
    ...
    state.tickets.forEach(ticket => {
        ...
        const card = document.createElement('div');
        card.className = `kanban-card severity-${ticket.severity}`;
    ...
    Object.keys(colCounts).forEach(k => {
        document.getElementById(`count-col-${k.replace('_', '-')}`).innerText = colCounts[k];
    });
}
